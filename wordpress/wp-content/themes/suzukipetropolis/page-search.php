...
<div id="primary">
	<main id="main" class="site-main mt-5" role="main">
		<suzuki-search class="suzuki-search row px-4">
			<suzuki-filters class="col-lg-3 col-md-3 col-sm-12 pb-4">
				<suzuki-clear-all-filters class="clear-all-filters"><button class="btn btn-secondary">Clear All filters</button></suzuki-clear-all-filters>
				<?php foreach ($filters_data as $filter_data) : ?>
					<suzuki-checkbox-accordion class="checkbox-accordion" key="<?php echo esc_attr($filter_data['slug'] ?? ''); ?>" label="<?php echo esc_attr($filter_data['label'] ?? ''); ?>">
						...
						<suzuki-checkbox-accordion-content class="checkbox-accordion__content">
							<?php foreach ($filter_data['children'] as $item) : ?>
								...
								<?php if (!empty($item['label'])) : ?>
									<suzuki-checkbox-accordion-child class="checkbox-accordion__child <?php echo esc_attr($has_content_class); ?>">
											...
											<div class="checkbox-accordion__child-content">
												<?php foreach ($item['children'] as $child) : ?>
													...
													<?php if (!empty($child['label']) && !empty($child['value'])) : ?>
														<suzuki-checkbox-accordion-child class="checkbox-accordion__child">
															<div class="checkbox-accordion__child-handle form-field">
								...
								<?php endif; ?>
							<?php endforeach; ?>
						</suzuki-checkbox-accordion-content>
					</suzuki-checkbox-accordion>
				<?php endforeach; ?>
			</suzuki-filters>
			<div class="col-lg-9 col-md-9 col-sm-12 pb-4">
				<suzuki-results-count class="suzuki-results-count"></suzuki-results-count>
				<suzuki-results class="row"></suzuki-results>
				<suzuki-loading-more class="suzuki-loading-more"></suzuki-loading-more>
			</div>
		</suzuki-search>
	</main>
</div>
