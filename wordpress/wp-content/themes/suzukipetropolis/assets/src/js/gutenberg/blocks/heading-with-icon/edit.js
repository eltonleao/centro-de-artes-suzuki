...
const Edit = (props) => {
  ...
  return (
    <div className="suzuki-icon-heading">
      <span className="suzuki-icon-heading__heading">
        <HeadingIcon />
      </span>
      ...
      <RichText
        ...
        placeholder={__("Heading…", "suzuki")} // Display this text before any content has been added by the user
      />
      <InspectorControls>
        <PanelBody title={__("Block Settings", "suzuki")}>
          <RadioControl
            label={__("Select the icon", "suzuki")}
            help={__("Controls icon selection", "suzuki")}
            selected={option}
            options={[
              { label: "Dos", value: "dos" },
